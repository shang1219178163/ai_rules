...
生成符合基本原则和命名规范的代码、修正和重构。通用原则（中文回复、禁止自动 git commit、少改少抽象）见 `common/core.md`，此处不重复。
...
- 优先高阶函数（`map` / `where` / `fold` 等）；简单（少于 3 条）用箭头函数，否则用具名函数。
- 用默认参数，而不是空检查凑默认值。
- 多参数 / 多返回值用对象（RO-RO）并声明类型。
- 保持单一抽象级别。
...
- 优先不可变；字面量用 `const`（Dart 无 `readonly` 关键字时用 `final` / `const` 表达不变性）。
- 遵循 SOLID；优先组合而非继承；用抽象类 / 接口表达契约。
- 小型类、功能单一；需要序列化的模型提供 `fromJson` / `toJson`。
- 类指令数少于 200；公共方法少于 10；属性少于 10。
...
- 组件：高内聚、低耦合、可复用、可扩展、可维护；参考 `lib/pages` 既有业务页风格。
- 页面 / 模块数据：集合类型放同级 `controller.dart`；非集合常量与文案放各方法 / 组件函数内部。
- 若模块有 controller：其中仅保存集合变量；其余转为函数本地变量。
- 字符串、图片路径等可复用数据用成员集合或变量集中管理，避免魔法散落。
...
- 代码审查或精简时不要删除代码注释。
- 注释写清复杂逻辑与非显而易见的决策；遵循官方 Flutter 文档最佳实践。
