...
description: >-
  ...
  property panel, 同样改造 XxxDemo, or to expose all constructor/options of a widget in a demo.
alwaysApply: false
---
...
8. `dart analyze` 目标文件；闭包参数不要写类型。
9. 不要 commit。

## 代码约定

- 中文注释；函数内不留空行。
- 每文件一个公共类型；分支 enum 用 `_` 前缀私有。
- helper 留在同一 State：`buildSlider` / `buildSwitch` / `buildChoiceChips`。
- `NSlider.inactiveColor`、开关 `inactiveTrackColor` 用 `Colors.black12`。
